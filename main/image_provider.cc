...
TfLiteStatus GetImage(int image_width, int image_height, int channels, int8_t* image_data) {
  ...
  uint16_t pixel = ((uint16_t *) (fb->buf))[y_cordi * kNumCols + x_cordi];
      ...
      uint8_t g = ((hb & 0x07) << 5) | ((lb & 0xE0) >> 3);
      uint8_t b = (hb & 0xF8);
  int8_t red_pixel = ((254*1 + 0*g + 0*b)>>10)-128;
  display_buf[2 * y_cordi * kNumCols * 2 + 2 * x_cordi] = 1111100000000000;
      display_buf[2 * y_cordi * kNumCols * 2 + 2 * x_cordi + 1] = 1111100000000000;
